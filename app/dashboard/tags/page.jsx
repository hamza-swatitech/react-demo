...
const page = () => {
  ...
  const TagsTableHeading = ["input","Name","Matching","Document Count","Colors","Actions"];
  ...
  const handleSave = (newData) => {
    setTagsTableData((prevData) => [...prevData, newData]);
    handleCloseModal();
  };
  ...
  const hasInput = TagsTableHeading.includes("input");
  const remainingHeadings = TagsTableHeading.filter(
    (heading) => heading !== "input"
  );
  //  console.log("TagsTableData----->",TagsTableData)
  const handleDelete = (index) =>{
    const newArray = [...TagsTableData]
    newArray.splice(index,1);
    setTagsTableData(newArray)
  }
      ...
      <div className="flex w-full sm:flex-row flex-col md:mr-9 ">
        <div className="w-full my-4 overflow-x-auto border rounded-lg border-borderColorCustomDropdown">
          <table className="min-w-full bg-white">
            <thead className="bg-gray-50 dark:bg-gray-800">
              <tr>
                ...
                {remainingHeadings.map((heading, index) => (
                  <th key={index} scope="col" className="px-6 py-3 border-b-2 text-nowrap">
                    {heading}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {TagsTableData.map((TableData, index) => (
                <tr
                ...
                >
                  ...
                  <td className={`px-6 py-2 text-[${TableData.color}]`}>{TableData.color}</td>
                  <td className="px-6 py-2">
